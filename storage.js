...
/**
 * Fetches AI insights from Gemini directly via the browser.
 * Handles anonymization and injects system instructions.
 *
 * ⚠️ WARNING: This method directly exposes your API key. Not recommended for production.
 */
async function fetchAIAssistance(taskType, payload) {
    const disclaimer = "Medical Disclaimer: This suggestion is generated by AI (Gemini) for educational purposes. It is not a substitute for professional medical advice, diagnosis, or treatment.";
    ...
    try {
        // The Gemini model configuration (e.g., model name, safety settings)
        // For health applications, strong safety settings are crucial.
        ...
        const chat = model.startChat({
            ...
            generationConfig: { maxOutputTokens: 200 }, // Keep responses concise
        });
...
/**
 * Specifically handles the CBT Reframing logic.
 */
async function getBalancedThoughtSuggestion(automaticThought) {
    const prompt = {
    ...
    return await fetchAIAssistance("cbt_reframing", prompt);
}

/**
 * Function 1: The "Spike" De-escalator (Crisis Support)
 */
async function getSOSSupport(userInput = "I am having a spike right now.") {
    const prompt = {
        system_instruction: "You are a specialized tinnitus crisis counselor. The user is experiencing a 'spike' (sudden increase in volume/distress). Provide immediate CBT reframing, reassure them that spikes are temporary physiological reactions, and suggest a 2-minute breathing focus. Keep it calm, supportive, and very brief.",
        input: userInput
    };
    return await fetchAIAssistance("sos_support", prompt);
}

/**
 * Function 2: Adaptive Soundscape Designer
 */
async function getSoundRecipe(description) {
    const prompt = {
        system_instruction: "You are a sound therapy expert. Based on the user's description of their tinnitus (e.g., 'hissing', 'roaring'), suggest a 'Sound Recipe' using noise colors (White, Pink, Brown) and nature sounds (Rain, Ocean, Stream, Wind). Provide a specific mixing ratio (e.g., 40% Brown, 20% Stream) and a one-sentence explanation of why it works for that sound.",
        input: description
    };
    return await fetchAIAssistance("sound_recipe", prompt);
}

/**
 * Function 3: Intelligent Log Correlation (Pattern Finder)
 */
async function getPatternAnalysis(userQuery = "") {
    const usage = getJson('usage_log', {});
    const distress = getJson('distress_log', {});
    
    // Include recent situational context from thought records to find environmental triggers
    const recentThoughts = getThoughtRecords().slice(-5).map(tr => ({
    ...
    const anonymizedHistory = {
        ...
        distress_history: distress,
        recent_situational_contexts: recentThoughts
    };

    const prompt = {
        system_instruction: "You are a specialized data analyst for tinnitus habituation. Review the user's therapy logs and situational data. Identify potential correlations (e.g., 'Distress is 20% lower on days you use the suite for >30 mins' or 'Your recorded situations suggest quiet environments are a trigger'). State clearly that these are correlations, not medical facts, and suggest one specific focus area for next week.",
        input: {
            user_question: userQuery, // Include the user's specific question
            anonymized_data: anonymizedHistory
        }
    };
    return await fetchAIAssistance("pattern_finder", prompt);
}

/**
 * Function 4: TRT Educator
 */
async function getTRTExplanation(question) {
    const prompt = {
        system_instruction: "You are a Tinnitus Retraining Therapy (TRT) coach. Explain audiology concepts like 'habituation', 'neuroplasticity', and the 'mixing point' in supportive, everyday language. Your goal is to reduce fear and promote the idea that the brain can learn to ignore the sound.",
        input: question
    };
    return await fetchAIAssistance("trt_coach", prompt);
}

/**
 * Function 5: Clinical Report "Translator"
 */
async function getClinicalSummary() {
    const reportData = getClinicalReportData("AI Summary Generation", {}, {});
    const condensedData = {
        distress_trend: reportData.psychological.lastTHIScore,
        usage: reportData.usage.todayMinutes,
        thought_records: reportData.psychological.thoughtRecordsCount
    };

    const prompt = {
        system_instruction: "You are a clinical assistant writing for an audiologist. Summarize the user's progress over the last period based on the data. Focus on trends in distress scores and therapy adherence. Write one professional paragraph (3-4 sentences) that can be included in a medical report.",
        input: condensedData
    };
...
/**
 * Analyzes the clinical report for patterns.
 */
async function getPersonalizedInsights() {
    const report = getClinicalReportData("Insight Generation", {}, {});
    
    // Anonymize for privacy: Remove specific dates and raw location data
    const anonymizedData = {
        thi_history: report.psychological.lastTHIScore,
        usage_stats: report.usage,
        recent_thought_count: report.psychological.thoughtRecordsCount,
        ri_results: report.ri.latestRIResult
    };

    const prompt = {
        system_instruction: "Analyze the following tinnitus therapy data. Look for positive trends in usage and score improvements. Provide one paragraph of encouragement and one actionable tip based on the data provided.",
        input: anonymizedData
    };

    return await fetchAIAssistance("progress_insights", prompt);
}
