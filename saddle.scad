...
// --- Export Control ---
// Allows CLI to export individual sides using: -D 'export_mode="left"'
export_mode = "both"; 

if (export_mode == "left" || export_mode == "both") {
    color("#00bfa5") 
        translate(export_mode == "both" ? [-25, 0, 0] : [0,0,0]) 
            saddle_v3(true);
}

if (export_mode == "right" || export_mode == "both") {
    color("#00897b") 
        translate(export_mode == "both" ? [25, 0, 0] : [0,0,0]) 
            mirror([1,0,0]) saddle_v3(false);
}
